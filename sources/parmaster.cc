...
int debugLevel_ = 0;		//!< currently global!
    ...
    idleCowTimeLast_(master)
  {
    ...
    if (ACE_OS::hostname(help, 100)==-1) {
      char *_error="ABA_PARMASTER::ABA_PARMASTER(): can't determine my hostname.";
      THROW_PARAM(AlgorithmFailureException,afcParmaster,_error);
    }
    myHostname_ = help;
  ...
  void ABA_PARMASTER::initializeParameters()
  {
    ...
    for (int i=0; i<hostCount_; i++) {
      ...
      if (myHostname_ == hostname(i)) {
        if (hostId_>=0) {
          char *_error="ABA_PARMASTER::initializeParameters(): host is included in .abacus more than once!"
          THROW_PARAM(AlgorithmFailureException,afcParmaster,_error);
        }
        hostId_=i;
      }
    ...
    if (hostId_==-1) {
      char *_error="ABA_PARMASTER::initializeParameters(): host is missing in .abacus!";
      THROW_PARAM(AlgorithmFailureException,afcParamaster,_error);
    }

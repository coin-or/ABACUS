...
#include <math.h>
  ...
  const ABA_SPARVEC& ABA_SPARVEC::operator=(const ABA_SPARVEC& rhs)
  {
	...
	sprintf(_error,"ABA_SPARVEC::operator= : length of operands are different (%i != %i ).\n", size_,rhs.size_);
        THROW_PARAM(AlgorithmFailureException,afcSparVec,_error);
    }
...
ostream& operator<<(ostream &out, const ABA_SPARVEC &rhs)
{
  ...
  void ABA_SPARVEC::leftShift(ABA_BUFFER<int> &del)
  {
	...
	char *_error=new char[255];
	sprintf(_error,"ABA_BUFFER:leftShift(): shift index  %i  not valid.",i); 
	THROW_PARAM(AlgorithmFailureException,afcSparVec,_error);
      }
#endif      
  ...
  void ABA_SPARVEC::realloc(int newSize)
  {
    if (newSize < nnz_) {
      char *_error=new char [255];
	sprintf(_error,"ABA_SPARVEC::realloc(%i):\nlength of vector becomes less than number of nonzeros %i",newSize,nnz_);
        THROW_PARAM(AlgorithmFailureException,afcSparVec,_error);
    }
  ...
  void ABA_SPARVEC::rangeCheck(int i) const
  {
    if (i < 0 || i >= nnz_) {
      char *_error=new char[255];
	sprintf(_error,"ABA_SPARVEC::rangeCheck(): index %i\nout of ranges of nonzeros [0,%i-1 ].",i,nnz_);
	THROW_PARAM(AlgorithmFailureException,afcSparVec,_error);
    }
  }

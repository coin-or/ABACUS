...
ABA_LPSUB *ABA_SUB::generateLp()
  {
    switch (master_->defaultLpSolver()) {
	  ...
	  char *_error=new char[255];
		sprintf(_error,"Error: ABACUS library not compiled for\nselected LP-Solver %s",ABA_MASTER::OSISOLVER_[master_->defaultLpSolver()] );
	THROW_PARAM(AlgorithmFailureException,afcLpIf,_error);
    }

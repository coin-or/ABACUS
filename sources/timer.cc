...
#include "abacus/exceptions.h" 
  ...
  void ABA_TIMER::start(bool reset)
  {
    if (reset)
      ...
      if (running_) {
        const char *_error="ABA_TIMER::start(): you cannot start a running timer.";
	THROW_PARAM(AlgorithmFailureException,afcTimer,_error);
      }
    running_   = true;
    startTime_ = theTime();
  }
  ...
  {
    if(!running_) {
      const char *_error="ABA_TIMER::stop : you cannot stop a non-running timer.";
      THROW_PARAM(AlgorithmFailureException,afcTimer,_error);
    }

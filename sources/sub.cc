 ...
 *
 * $Id: sub.cc,v 2.12 2007/08/14 13:09:26 baumann Exp $
 */
...
/* The local variable \a bits is unpacked. Each bit set indicates that an
 ...
 */  
  ...
  // FIXME FixAndSet approx
  if (master_->primalViolated(dualRound(lp_->value()))) {
     // FIXME Do not do FixAndSet if the LP was solved approximate
    status = _pricing(newValues, lastLP_ != ABA_LP::Approximate );
    if (status)       continue;
 ...
 */  
  ...
  bool dummy;     //!< required to call function \a set(), no new values possible here
  int i;          //!< loop index

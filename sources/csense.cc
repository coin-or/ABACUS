    ...
    glob_(glob)
  {
    switch (s) {
      ...
      default: char *_error=new char[255];
	       sprintf(_error,"ABA_CSENSE::ABA_CSENSE(): unknown argument %c ", s);
               THROW_PARAM(AlgorithmFailureException,afcCsense,_error);
               break;
    }
  }
  ...
  void ABA_CSENSE::sense(char s)
  {
    switch (s) {
      ...
      case 'G': sense_ = Greater;
                break;
      default: char *_error=new char[255];
               sprintf(_error,"ABA_CSENSE::sense(): unknown argument %c ", s); 
               THROW_PARAM(AlgorithmFailureException,afcCsense,_error);
               break;

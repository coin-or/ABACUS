...
#include "abacus/exceptions.h"
    ...
    value_(0.0)
  {
    if (status == Fixed || status == Set) {
      const char *_error="ABA_FSVARSTAT::ABA_FSVARSTAT(): value to set/fix missing";
      THROW_PARAM(AlgorithmFailureException,afcFsVarStat,_error);
    }
  }
  ...
  {
    if (status != Fixed && status != Set) {
      const char *_error= "ABA_FSVARSTAT::ABA_FSVARSTAT(): wrong status for this constructor";
      THROW_PARAM(AlgorithmFailureException,afcFsVarStat,_error);
    }
  }

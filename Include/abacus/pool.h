...
/*! Checks if a constraint/variable with the supplied ABA_ID exists in the pool.
 *
 * \return A pointer to the poolslot of the constraint/variable if it exists in the
 * pool, otherwise 0.
 */
      ABA_POOLSLOT<BaseType, CoType> *findSlot(const ABA_ID &id) const;

/*! \return A pointer to the ABA_IDMAP of the ABA_POOL of this processor.
 */
      ABA_IDMAP<ABA_POOLSLOT<BaseType, CoType>   > *identificationMap() const;
#endif
 ...
 */
      ...
      int number_;
#ifdef ABACUS_PARALLEL
      ABA_IDMAP<ABA_POOLSLOT<BaseType, CoType>   > *identificationMap_;
      int index_;
#endif
  };

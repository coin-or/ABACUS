 ...
 *
 * $Id: master.h,v 2.12 2009-05-13 13:25:44 baumann Exp $
 */
...
template<class BaseType, class CoType> class ABA_STANDARDPOOL;
  ...
  class  ABA_MASTER :  public ABA_GLOBAL  {
     ...
     * \param MaxNSub The status, if the optimization terminates since
     *                     the maximum number of subproblems is reached.
 ...
 *************************************************************************/
       ...
       */
      enum OSISOLVER {Cbc, Clp, CPLEX, DyLP, FortMP, GLPK, MOSEK, OSL, SoPlex, SYMPHONY, XPRESS_MP, Csdp};
...
/*! \return the dual bound at the root node
 */
...
/*!  \brief The number of simplex iterations that are performed
 *          when testing candidates for branching variables within
 *          strong branching.
 */     
      int nStrongBranchingIterations() const;

/*! \brief This version of the function \a nStrongBranchingIterations()
 *         sets the number of simplex iterations that are performed
 *         when testing candidates for branching variables within
 *         strong branching.
 *
...
/*! \return The maximal number of subproblems
 *   to be processed.
 *   By default this number is \a INT\_MAX. 
 */
      int maxNSub() const;

/*! \brief This version of the function \a maxNSub()
 *   changes the maximal number of subproblems.
 *
 *   If it is set to 1
 *   the \bac\ algorithm becomes a pure cutting plane algorithm.
 *
 ...
 *  (with integer values).
 *  
 *   \param hour, min, sec new value of the maximal cpu time.
 */    
      void setCpuTime(const int &hour, const int &min, const int &sec);
...
/*! \brief Assigns the parameters that were read from a file to
 * 		   the member variables of the master.
 */
 ...
 */
      ...
      */
      ...
      /*! \brief The number of simplex iterations that are performed
       *  when testing a branching variable candidate within strong 
       *  branching.
      */
...
inline int ABA_MASTER::nStrongBranchingIterations() const
  {
    return nStrongBranchingIterations_;
  }
...
inline int ABA_MASTER::maxNSub() const
  {
    return maxNSub_;
  }

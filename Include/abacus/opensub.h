...
/*! Terminates the function \a select().
 */
      void terminate();

/*! \brief Extracts the best subproblem
 *   of the list of open subproblems if its bound is
 *   at least as good as the given value.
 *
 *   \param bound The requested dual bound}
 *
 *   \return A pointer to such a subproblem or 0.
 */
      ABA_SUB *getSubproblemWithBound(double bound);
      void triggerSelect();
#endif
...
#ifdef ABACUS_PARALLEL
inline void ABA_OPENSUB::triggerSelect()
  {
    cond_.signal();
  }
#endif
